...
function createDot(index, data) {
    ...
    index !== 1 ? el.className = 'news-pagination__item' : el.className = 'news-pagination__item active';
    el.dataset.page = data;
    el.innerHTML = data;
    return el
}
...
function checkedPage(pages, conf) {
    ...
    $('.news-pagination__item.active').removeClass('active');
    hideDots(page);
    changePage(page, conf);
    $('.news-pagination__item[data-page=' + paginationDefaults.currentPage + ']').addClass('active');
}

function hideDots(page) {
    $('.news-pagination__item').map(function(i, elem) {
        $(elem).show();
        if (i >= page.last) {
            $(elem).hide();
        }
...
function changeDots(page, last) {
    ...
    if (last >= 3) {
        ...
        //     currentDots.first = 1;
        //     $('.news-pagination__item').map( (i,li) => {
        //         li.dataset.page =  i;
        //         li.innerHTML =  i;
        //         currentDots.last = i ;
        //     })
